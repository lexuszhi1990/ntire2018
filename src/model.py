...
class LapSRN_v3(object):
  ...
  def current_step_img_size(self, step):
    ...
    current_height, current_width = tf.Tensor.get_shape(current_tensor).as_list()
  ...
  def extract_features(self, reuse=False):
    with tf.variable_scope(self.scope) as vs:
      ...
      for step in range(1, self.step_depth+1):
        upscaled_height, upscaled_width = self.current_step_img_size(step-1)
        base_images = tf.image.resize_bicubic(base_images, size=[upscaled_height, upscaled_width], align_corners=False, name='level_{}_biliear'.format(str(scale)))
