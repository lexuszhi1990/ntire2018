...
class LapSRN_v2(object):
  ...
  def extract_features(self, reuse=False):
    with tf.variable_scope(self.scope) as vs:
      ...
      for scale in range(2, self.upscale_factor+1):

        with tf.variable_scope('level_{}_img'.format(str(scale))):
          # current width and height for current stage.
          height = self.height*scale.astype(int)
          width = self.width*scale.astype(int)
        ...
        net = layers.conv2d(x, 1, kernel_size=self.kernel_size, stride=1, padding='SAME', activation_fn=lrelu, biases_initializer=None, weights_regularizer=layers.l2_regularizer(scale=self.reg), scope='level_{}_img'.format(str(scale)))

        self.extracted_features.append(net)

      base_images = self.inputs
      for scale in range(2, self.upscale_factor+1):
        upscaled_width = self.width*scale.astype(int)
        upscaled_height = self.height*scale.astype(int)
  ...
  def reconstruct(self, reuse=False):
    with tf.variable_scope(self.scope) as vs:
      ...
      for l in range(self.upscale_factor-1):
        sr_img = tf.add(self.reconstructed_imgs[l], self.extracted_features[l])
        self.sr_imgs.append(sr_img)
